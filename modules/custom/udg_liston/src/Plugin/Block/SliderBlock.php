...
class SliderBlock extends BlockBase {
    ...
    public function build() { 
		...
		//kint($result);
		$nids = \Drupal::entityQuery('node')->condition('status', 1)->condition('type','slideshow')->execute();
		$nids2 =  \Drupal::entityQuery('node')->condition('status', 1)->condition('type','video')->execute();
		...
		foreach ($nodesVideo as $node){
			...
			foreach($node->get('field_video')->getValue() as $file){
				//kint($file);
				$fid = $file['target_id'];
				$video = file_create_url(\Drupal\file\Entity\File::load($fid)->getFileUri());
				//kint($video);
